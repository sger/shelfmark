...
pub async fn update_book(_user: CurrentUser, State(state): State<AppState>, Path(id): Path<Uuid>, Json(input): Json<UpdateBookRequest>) -> AppResult<Json<Book>> {
    ...
    .bind(input.authors.unwrap_or(existing.authors))
    .bind(input.description.or(existing.description))
    .bind(input.publisher.or(existing.publisher))
    .bind(input.published_date.or(existing.published_date))
    .bind(input.language.or(existing.language))
    .bind(input.page_count.or(existing.page_count))
    .bind(id)
    .fetch_one(&state.db)
    .await?;
    Ok(Json(book))
...
pub async fn delete_book(_user: CurrentUser, State(state): State<AppState>, Path(id): Path<Uuid>) -> AppResult<Json<serde_json::Value>> {
    sqlx::query("DELETE FROM books WHERE id = $1").bind(id).execute(&state.db).await?;
